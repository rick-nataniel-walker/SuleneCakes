...
function getHeaders() {
  return {"Authorization": `Bearer ${sessionStorage.getItem("token")??"12|I0Z8A8fjvY8WciOpBfAKYN16HQGeRHmSFHWtyUAT"}`};
}
...
function tableFiller(htmlAreaId,payload,callback){
  ...
  table===null?table=null:table.innerHTML="";
 ...
 * @returns {{list: *[], hashMap: {}}}
 */
function storeData(url, form, METHOD='postAjax') {
  return new Promise((resolve, reject) => {
    let list = [];
    let hashMap = null;
    if (typeof window[METHOD] === 'function') {
      window[METHOD]((serverData, status, error) => {
        if (typeof serverData === undefined) reject (new Error("Error interno"));
        if (!serverHasErrors(serverData, status, error)) {
